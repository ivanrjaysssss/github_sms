...
function updatelinkeid(){
	
	
	
	var linkeid = document.getElementById("linkeid").value;
	var linkuid = document.getElementById("linkuid").value;
	
	$.ajax({
		url: 'users/updatelinkeid',
		type: 'post',
		data: {linkeid : linkeid,linkuid:linkuid},
		success: function(response) {
			//console.log(response);
			 //var data = JSON.parse(response);
			 window.location.reload();
...
function linkeidmodal(uid){
	document.getElementById("linkuid").value = uid;
